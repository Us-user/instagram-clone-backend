...
## 📍 Текущий статус
- **Активная фаза:** Phase 19 — Верификация (Admin) (§10) — **не начата**. Phase 18 (real-time статусы: presence + typing) завершена.
- **Последняя сессия:** 2026-07-14 (22) — реализована Phase 18 (§1): presence + typing. `User.LastSeen (DateTime?)` + миграция `AddUserLastSeen` (одна колонка в `AspNetUsers`); in-memory singletons `PresenceTracker` (онлайн = ≥1 активное соединение любого хаба, счёт connectionId) и `TypingTracker` (эфемерный список печатающих по группам, TTL 6с); `PresenceService` — статусы со **взаимной** `ShowOnlineStatus` (запрашивающий скрыл → не видит чужие и сам скрыт; + блок/скрытие цели), переходы онлайн/офлайн (обновление `LastSeen` + рассылка); `TypingService` (best-effort, без БД: участие/членство + блок-проверка, kind∈{text,voice}); базовый `PresenceAwareHub<T>` — все 4 хаба (chat/group/notification/presence) кормят presence через `OnConnected/OnDisconnected`; `ChatHub.Typing(chatId,kind)` → `UserTyping` собеседнику, `GroupChatHub.Typing(groupChatId,kind)` → `GroupTyping` (список) остальным; новый `/presenceHub` + `IPresenceClient.ReceivePresence`; `PresenceController` (`/Presence/get-status`, `/Presence/get-statuses`); нотификаторы `PresenceNotifier`/`TypingNotifier`; сид `LastSeen` для bob/carol.
- **Следующий шаг:** начать Phase 19 — верификация (Admin): `AdminController` `[Authorize(Roles="Admin")]`, эндпоинты `POST /Admin/verify-user`, `DELETE /Admin/unverify-user`, `POST /Admin/grant-admin`, `DELETE /Admin/revoke-admin` (`User.IsVerified` уже есть с Phase 11; `isVerified` уже отдаётся в DTO — миграция может не потребоваться).
- **Состояние сборки:** 🟢 зелёная (0 предупреждений; база 96 операций эндпоинтов: 94 + 2 Phase 18: `/Presence/get-status` + `/Presence/get-statuses`; presence/typing real-time — по SignalR, не REST). Проверено: сборка, offline-скрипт миграции (`ALTER TABLE "AspNetUsers" ADD "LastSeen" timestamp with time zone` — единственное изменение, корректно), boot приложения (БД недоступна → ошибка миграции поймана, хост поднялся на :5030, DI валиден) + регистрация обоих `/Presence/*` эндпоинтов и `/presenceHub` в приложении. Живой smoke-тест на PostgreSQL/SignalR в эту сессию не гонялся (нет доступа к `DATABASE_URL`), запускается при деплое.
