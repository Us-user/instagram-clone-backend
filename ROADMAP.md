...
## 📍 Текущий статус
- **Активная фаза:** Phase 20 — Двухфакторная аутентификация (§11) — **не начата**. Phase 19 (верификация Admin) завершена.
- **Последняя сессия:** 2026-07-14 (23) — реализована Phase 19 (§10): верификация (Admin). `IAdminService`/`AdminService` + `AdminController` (`[Authorize(Roles = DbInitializer.AdminRole)]`, тег в Swagger) с 4 эндпоинтами: `POST /Admin/verify-user`, `DELETE /Admin/unverify-user` (флаг `User.IsVerified` через `UserManager.UpdateAsync`), `POST /Admin/grant-admin`, `DELETE /Admin/revoke-admin` (роль Admin через `AddToRoleAsync`/`RemoveFromRoleAsync`); проверки «уже в целевом состоянии» → `BadRequest`, защита от самолокаута (нельзя снять админа с себя), `userId` из query, текущий админ из claims. **Миграция не потребовалась** — колонка `IsVerified` уже есть с Phase 11. Ключевая доработка: `isVerified` **дозаведён во все базовые author-DTO**, которые его не отдавали, — `GetUserProfileDto`, `GetPostDto`, `GetPostCommentDto`, `GetStoryDto`, `GetUserDto` (проекции `PostProjections`/`CommentProjections`/`StoryProjections` + ручные `new …Dto` в `UserProfileService`/`UserService`/`BlockService`/`FollowingRelationShipService`). Сид: `admin` и `alice` — `IsVerified=true`.
- **Следующий шаг:** начать Phase 20 — 2FA (§11): `User.TwoFactorEnabled`/`TwoFactorSecret`, сущность `BackupCode`, TOTP (`Otp.NET`/Identity) + email-код + резервные коды; флоу `/Account/login` → `twoFactorToken` → `POST /Account/login-2fa`; эндпоинты enable/confirm/disable/send-2fa-email/regenerate-backup-codes; миграция.
- **Состояние сборки:** 🟢 зелёная (0 предупреждений; 100 операций эндпоинтов: 96 + 4 Phase 19 `/Admin/*`). Проверено: сборка, boot приложения (БД недоступна → ошибка миграции/Seed поймана, хост поднялся на :5030, DI валиден при `ValidateOnBuild`), все 4 `/Admin/*` эндпоинта присутствуют в `swagger.json` (96 → 100 операций). Живой smoke-тест на PostgreSQL (реальные verify/grant + отрисовка `isVerified` в DTO) в эту сессию не гонялся (нет доступа к `DATABASE_URL`), запускается при деплое.
