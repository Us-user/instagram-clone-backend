...
## 📍 Текущий статус
- **Активная фаза:** Phase 21 — Explore / рекомендации (§12) — **не начата**. Phase 20 (2FA) завершена.
- **Последняя сессия:** 2026-07-14 (24) — реализована Phase 20 (§11): двухфакторная аутентификация (TOTP + email-код + резервные коды). Домен: `User.TwoFactorSecret` (флаг `TwoFactorEnabled` — стандартный из `IdentityUser`), сущность `BackupCode`, enum `TwoFactorMethod`, DTO `Login2FaDto`/`Send2FaEmailDto`/`TwoFactorRequiredDto`/`Enable2FaResultDto`. Инфраструктура: `ITotpService`/`TotpService` (собственный RFC 6238 на `HMACSHA1` + Base32, проверен на 5 тест-векторах RFC), `ITwoFactorTokenStore`/`TwoFactorTokenStore` (in-memory singleton: токены login-сессии + email-коды, TTL 10 мин), `BackupCodeHasher` (SHA-256 + нормализация). `AccountService` расширен 7 методами (login теперь `Response<object>` с 2FA-ветвью). Контроллер: 6 новых эндпоинтов (`login-2fa`/`send-2fa-email` — аноним; `enable`/`confirm`/`disable`/`regenerate-backup-codes` — авторизованы). Миграция `Add2FA` (колонка + таблица `BackupCodes`). Сид: демо-аккаунт `frank` с включённой 2FA + известными резервными кодами.
- **Следующий шаг:** начать Phase 21 — Explore/рекомендации (§12): профиль интересов (веса favorite>comment>like>view + затухание), агрегация любимых хэштегов/авторов, скоринг кандидатов, фильтры (свои/просмотренные/блок/приват/уже подписан), разнообразие, эндпоинты `GET /Explore/get-feed` и `GET /Explore/get-popular` (cold start).
- **Состояние сборки:** 🟢 зелёная (0 предупреждений; 106 операций эндпоинтов: 100 + 6 Phase 20 `/Account/*-2fa` и др.). Проверено: сборка, миграция `Add2FA` сгенерирована (только `TwoFactorSecret` + `BackupCodes`, без лишнего `TwoFactorEnabled`), boot приложения (БД недоступна → ошибка Seed поймана, хост поднялся на :5030, DI валиден при `ValidateOnBuild`), все 6 новых эндпоинтов присутствуют в `swagger.json` (100 → 106 операций), TOTP проверен на официальных тест-векторах RFC 6238 (5/5 PASS + Base32 round-trip). Живой smoke на PostgreSQL (реальный enable→confirm→login-2fa) в эту сессию не гонялся (нет `DATABASE_URL`) — но login-флоу через `frank`+резервный код готов к проверке при деплое.
