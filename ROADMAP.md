...
## 📍 Текущий статус
- **Активная фаза:** Phase 17 — Сторис: close friends, ответы, репост поста — **не начата**. Phase 16 (реакции/reply/forward/голосовые) завершена.
- **Последняя сессия:** 2026-07-14 (20) — реализована Phase 16 (§8): enum `MessageContext {Direct, Group}`; полиморфная сущность `MessageReaction` (MessageId → `Message`/`GroupMessage` по контексту, без FK на сообщение, unique `(MessageId, MessageContext, UserId)`); расширен личный `Message` (`MessageType`/`Duration`/`Waveform`/`ReplyToMessageId` self-ref `SET NULL`/`IsForwarded`); новый `MessageController` (`/Message/react`, `/Message/forward`, `/Message/send-voice`) поверх `MessageService`/`IMessageService`; реакции (тумблер/замена) с блок-проверкой Direct и членством Group, запрет на служебные, real-time `ReceiveReaction` на обоих хабах; forward = физическая копия файла + `IsForwarded=true`; голосовые в `wwwroot/voice` (FileService расширен параметром папки + `CopyFile`), детерминированный `WaveformGenerator`; `ReactionEnrichment` раскладывает реакции по DTO сообщений (`Reactions[]` в `GetMessageDto`/`GetGroupMessageDto`); reply-цитата в личных сообщениях + `ReplyToMessageId` в `POST /Chat/send-message`; очистка реакций при удалении сообщения/чата; валидатор `SendVoiceDto`; миграция `AddMessageReactionsAndVoice` + сид (реакции в группе admin ❤️/bob 🔥, личный чат alice↔bob с reply и реакцией bob 😂).
- **Следующий шаг:** начать Phase 17 — сторис close friends/ответы/репост поста: сущности `CloseFriend`, `Story.Audience (All/CloseFriends)`, `Story.SharedPostId`, `StoryReply`; эндпоинты `/CloseFriend/*`, `/Story/reply`, `/Story/share-post`; фильтрация close-friends в выдачах сторис; уведомления `StoryReply`/`PostShared`; миграция.
- **Состояние сборки:** 🟢 зелёная (0 предупреждений; база 89 операций эндпоинтов: 86 + 3 `/Message/*`). Проверено: сборка, offline-скрипт миграции (`MessageReactions` + 5 колонок в `Messages`, User FK каскадом, unique `(MessageId, MessageContext, UserId)`, self-ref reply `ON DELETE SET NULL` — SQL корректен), boot приложения (БД недоступна → ошибка миграции поймана, хост поднялся на :5030) + регистрация всех 3 `/Message/*` в Swagger (send-voice как multipart-форма: Context/ChatId/File/Duration/ReplyToMessageId). Живой smoke-тест на PostgreSQL в эту сессию не гонялся (нет доступа к `DATABASE_URL`), запускается при деплое.
