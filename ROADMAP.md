...
## 📍 Текущий статус
- **Активная фаза:** Phase 22 — Качество/харденинг/документация (§13) — **не начата**. Phase 21 (Explore) завершена.
- **Последняя сессия:** 2026-07-14 (25) — реализована Phase 21 (§12): Explore / рекомендации (content-based, без ML). Профиль интересов считается **на лету** (без сущности `UserInterest`): `ExploreService.BuildInterestProfileAsync` собирает веса постов из взаимодействий (favorite=4>comment=3>like=2>view=1) с экспоненциальным затуханием (полураспад 30 дней) и разворачивает их в веса любимых хэштегов и авторов. Скоринг кандидата `0.40·хэштеги + 0.30·автор + 0.15·популярность + 0.15·свежесть` (компоненты нормируются в [0..1] по максимуму пула; свежесть — затухание с полураспадом 7 дней). Фильтры: свои посты, просмотренные (`PostView`), блок/приват (`AccessGuard.VisibleTo`), уже-подписан (`Accepted`). Разнообразие: ≤2 подряд от автора (`Diversify`). Пул кандидатов ограничен 500 свежайшими, ранжирование in-memory, страница материализуется через `PostProjections.ToDto` + `MentionEnrichment` (порядок скоринга сохраняется). `IExploreService`/`ExploreService` + `ExploreController` (`GET /Explore/get-feed`, `GET /Explore/get-popular`), DI. Миграции нет (профиль на лету). Сид расширен Explore-контентом вокруг alice.
- **Следующий шаг:** начать Phase 22 — качество/харденинг (§13): сквозная проверка блок/приват во всех новых выдачах (в т.ч. сторис — задел из Phase 17), ревизия «не себе» для уведомлений/mention, ревизия авторизации владельца/ролей/групп-ролей, пагинация+валидаторы на всех новых DTO, обновление README (новые эндпоинты/фичи), финальный smoke на живом PostgreSQL, сборка без предупреждений.
- **Состояние сборки:** 🟢 зелёная (0 предупреждений; **108** операций эндпоинтов: 106 + 2 Phase 21 `/Explore/get-feed`, `/Explore/get-popular`). Проверено: сборка, boot приложения (БД недоступна → ошибка Seed поймана, хост поднялся на :5030, DI валиден), оба новых эндпоинта присутствуют в `swagger.json` (106 → 108 операций), алгоритм скоринга проверен offline-симуляцией на сид-данных (лента alice: `carolTravel` > `carolSunset` > `frankTravel` — совпадение интересов + близость автора + популярность/свежесть, разнообразие соблюдено). Живой smoke на PostgreSQL (реальный `get-feed`/`get-popular` под `alice`) в эту сессию не гонялся (нет `DATABASE_URL`) — сид готовит проверяемый сценарий при деплое.
